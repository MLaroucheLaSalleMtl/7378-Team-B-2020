		...
		*/
		...
		void Avoid_Obstacle () {
			...
			for (int i = 0; i < raycastHits.Length; i++) {
				if (raycastHits [i].transform.root != rootTransform) { // The hit object is not myself.
					if (isAvoidingObstacle == false) { // Not avoiding now.
					...
					else if (raycastHits [i].distance < storedDistance) { // Find a new obstacle that is closer to the tank.
						targetDistance = raycastHits [i].distance;
						targetDistance = Mathf.Clamp (targetDistance, Min_Dist, Max_Dist);
					}
					return;
		...
		void Move()
		{
			...
			if (currentDistance != targetDistance) {
				currentDistance = Mathf.MoveTowards (currentDistance, targetDistance, Move_Speed * Time.fixedDeltaTime);
