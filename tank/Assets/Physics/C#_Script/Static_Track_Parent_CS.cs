	...
	public class Static_Track_Parent_CS : MonoBehaviour
	{
		...
		*/
		...
		void Initialize ()
		{
			...
			if (Reference_L == null) { // The left reference wheel has been lost by modifying.
			...
			if (Reference_R == null) { // The right reference wheel has been lost by modifying.
			...
			Static_Wheel_Parent_CS[] staticWheelParentScripts = bodyTransform.GetComponentsInChildren <Static_Wheel_Parent_CS>();
		...
		{
			if (bodyScript) {
				Is_Visible = bodyScript.Visible_Flag; // The MainBody is visible or not by any camera.
			}
			else {
				Is_Visible = true;
			}
