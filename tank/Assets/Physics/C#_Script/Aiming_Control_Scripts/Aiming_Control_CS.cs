	...
	public class Aiming_Control_CS : MonoBehaviour
	{
		...
		*/
		...
		{ // Find a target with a rigidbody by casting a ray from the main camera.
			...
			if (Physics.Raycast (ray, out raycastHit, 2048.0f, Layer_Settings_CS.Aiming_Layer_Mask)) {
		...
		{ // Cast a sphere to find a target with a rigidbody.
			...
			raycastHits = Physics.SphereCastAll (ray, spherecastRadius, 2048.0f, Layer_Settings_CS.Aiming_Layer_Mask);
		...
		{ // 0 = Left, 1 = Right, 2 = Front.
			...
			for (int i = 0; i < idScripts.Length; i++) {
				if (idScripts[i].Relationship == thisRelationship || idScripts[i].Relationship == 2) { // The tank is a friend, or it is a landmark.
					continue;
				}
