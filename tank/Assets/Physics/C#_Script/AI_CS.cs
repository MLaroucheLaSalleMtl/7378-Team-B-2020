...
namespace ChobiAssets.PTM
{
	...
	public class AI_CS : MonoBehaviour
	{
		...
		*/
        ...
        float updateDestinationPeriod = 0.5f;
		...
		const float followDistance = 15.0f;
        ...
        bool disableDeadAngle;
        int disableDeadAngleTimerID;
		...
		void Update()
		{
			...
			else
            {
                ...
                navAgent.speed = Mathf.Lerp(128.0f, 0.0f, navAgentDistance / (Agent_Distance + (Agent_Additional_Distance * tempRate)));
			}
		...
		void Search_Target()
		{
            ...
            if (targetDistance < Settings_Script.Visibility_Radius)
            { // The target is within the "Visibility_Radius".
		...
		bool Cast_Line_For_Searching(Vector3 targetPosition)
		{
            ...
            if (Action_Type == 0 && disableDeadAngle == false)
        ...
        public IEnumerator Disable_Dead_Angle_Timer()
        { // Called from "Damage_Control_CS", when the AI tank is attacked.
            disableDeadAngle = true;
            disableDeadAngleTimerID += 1;
            int thisTimerID = disableDeadAngleTimerID;

            float count = 0.0f;
            while (count < 5.0f)
            {
                if (thisTimerID != disableDeadAngleTimerID)
            ...
            disableDeadAngleTimerID = 0;
            disableDeadAngle = false;
        }
		...
		{
            // Get the angle to the NavMeshAgent, and the distance to the next corner.
