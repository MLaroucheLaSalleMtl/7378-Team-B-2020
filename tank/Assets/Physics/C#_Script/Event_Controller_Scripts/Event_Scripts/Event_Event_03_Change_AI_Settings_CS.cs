	...
	public class Event_Event_03_Change_AI_Settings_CS : Event_Event_00_Base_CS
	{
		...
		public override void Execute_Event()
		{
			for (int i = 0; i < eventControllerScript.Target_Tanks.Length; i++) {
				...
				if (aiSettingsScript) { // The tank has been spawned.
				...
				if (targetEventScript) { // The tank has not been spawned yet.
			...
			if (eventControllerScript.Trigger_Script == null) { // All the triggers are pulled.
