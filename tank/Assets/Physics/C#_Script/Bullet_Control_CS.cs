		...
		*/
		...
		void AP_Hit_Process(GameObject hitObject, float hitVelocity, Vector3 hitNormal)
		{
			...
			if (damageScript != null) { // The hit object has "Damage_Control_##_##_CS" script. >> It should be a breakable object.
				...
				float hitAngle = Mathf.Abs(90.0f - Vector3.Angle(This_Transform.forward, hitNormal));
		...
		void HE_Hit_Process ()
		{
			...
			Collider[] colliders = Physics.OverlapSphere (This_Transform.position, Explosion_Radius, Layer_Settings_CS.Layer_Mask);
		...
		void Add_Explosion_Force (Collider collider)
		{
			...
			if (Physics.Raycast (ray, out raycastHit, Explosion_Radius, Layer_Settings_CS.Layer_Mask)) {
