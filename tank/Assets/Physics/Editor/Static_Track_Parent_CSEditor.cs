...
namespace ChobiAssets.PTM
{
	...
	public class Static_Track_Parent_CSEditor : Editor
	{
		...
		void Set_Inspector()
		{
            ...
            EditorGUILayout.Space();
            EditorGUILayout.Space();
            EditorGUILayout.HelpBox("Update the track pieces from version 2.5.", MessageType.None, true);
            if (GUILayout.Button("Update from 2.5", GUILayout.Width(200)))
            {
		...
		void Update_Track_Pieces ()
		{
			// Copy the values from the old parent "Static_Track_CS" script.
			Static_Track_CS oldParentScript = thisTransform.GetComponent <Static_Track_CS>();
			Static_Track_Parent_CS newParentScript = thisTransform.GetComponent <Static_Track_Parent_CS>();
			if (oldParentScript) {
				newParentScript.Reference_L = oldParentScript.Reference_L;
				newParentScript.Reference_R = oldParentScript.Reference_R;
				newParentScript.Reference_Name_L = oldParentScript.Reference_Name_L;
				newParentScript.Reference_Name_R = oldParentScript.Reference_Name_R;
				newParentScript.Reference_Parent_Name_L = oldParentScript.Reference_Parent_Name_L;
				newParentScript.Reference_Parent_Name_R = oldParentScript.Reference_Parent_Name_R;
				newParentScript.Length = oldParentScript.Length;
				newParentScript.Radius_Offset = oldParentScript.Radius_Offset;
				newParentScript.Mass = oldParentScript.Mass;
				newParentScript.Track_L_Shadow_Mesh = oldParentScript.Track_L_Shadow_Mesh;
				newParentScript.Track_R_Shadow_Mesh = oldParentScript.Track_R_Shadow_Mesh;
				newParentScript.RoadWheel_Effective_Range = oldParentScript.RoadWheel_Effective_Range;
				newParentScript.SwingBall_Effective_Range = oldParentScript.SwingBall_Effective_Range;
				newParentScript.Anti_Stroboscopic_Min = oldParentScript.Anti_Stroboscopic_Min;
				newParentScript.Anti_Stroboscopic_Max = oldParentScript.Anti_Stroboscopic_Max;
				newParentScript.RoadWheelsProp_Array = oldParentScript.RoadWheelsProp_Array;
				newParentScript.Stored_Body_Mass = oldParentScript.Stored_Body_Mass;

				// Remove the old parent script.
				DestroyImmediate(oldParentScript);
			}

			// Copy the values from the old child "Static_Track_CS" scripts.
			Static_Track_CS[] oldChildScripts = thisTransform.GetComponentsInChildren <Static_Track_CS>();
			for (int i = 0; i < oldChildScripts.Length; i++) {
				Static_Track_Piece_CS newChildScript = oldChildScripts[i].gameObject.AddComponent <Static_Track_Piece_CS>();
				// Copy the old values.
				newChildScript.Type = oldChildScripts[i].Type;
				newChildScript.Front_Transform = oldChildScripts[i].Front_Transform;
				newChildScript.Rear_Transform = oldChildScripts[i].Rear_Transform;
				newChildScript.Anchor_Name = oldChildScripts[i].Anchor_Name;
				newChildScript.Anchor_Parent_Name = oldChildScripts[i].Anchor_Parent_Name;
				// Set the "This_Transform".
				newChildScript.This_Transform = newChildScript.transform;
				// Set the "Parent_Script".
				newChildScript.Parent_Script = newParentScript;
				// Set the "Is_Left".
				newChildScript.Is_Left = (newChildScript.transform.localPosition.y > 0.0f);
				// Set the "Invert_Angle".
				float localAngleY = newChildScript.transform.localEulerAngles.y;
				if (localAngleY > 90.0f && localAngleY < 270.0f) { // Upper piece.
					newChildScript.Invert_Angle = 180.0f;
				} else { // Lower piece
					newChildScript.Invert_Angle = 0.0f;
				}
				// Set the "Half_Length".
				newChildScript.Half_Length = newParentScript.Length * 0.5f;
				// Set the "Pieces_Count".
				newChildScript.Pieces_Count = (oldChildScripts.Length) / 2;

				// Remove the old child script.
				DestroyImmediate (oldChildScripts[i]);
			}

			// Update the "Static_Track_Switch_Mesh_CS" scripts in the pieces.
			Static_Track_Switch_Mesh_CS[] switchScripts = thisTransform.GetComponentsInChildren <Static_Track_Switch_Mesh_CS>();
			for (int i = 0; i < switchScripts.Length; i++) {
				switchScripts[i].Piece_Script = switchScripts[i].GetComponent <Static_Track_Piece_CS>();
				switchScripts[i].Parent_Script = newParentScript;
				switchScripts[i].This_MeshFilter = switchScripts[i].GetComponent <MeshFilter>();
				switchScripts[i].Mesh_A = switchScripts[i].This_MeshFilter.sharedMesh;
				switchScripts[i].Mesh_B = switchScripts[i].Piece_Script.Front_Transform.GetComponent <MeshFilter>().sharedMesh;
				switchScripts[i].Is_Left = switchScripts[i].Piece_Script.Is_Left;
			}

			// Done.
			Debug.Log("The track pieces have been updated from 2.5.");

            // Set values of the new scripts.
            Update_Scripts_Values();
        }
