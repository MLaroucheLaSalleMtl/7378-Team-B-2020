...
namespace ChobiAssets.PTM
{
    ...
    public class UI_PosMarker_Control_CS : MonoBehaviour
    {
		...
		*/
        ...
        {
            if (idScript.Is_Selected || rootTransform.tag == "Finish")
            { // The tank is selected now, or has been dead.
                    ...
                    { // AI tank.
                        ...
                        switch (aiScript.Action_Type)
                        {
                            case 0: // Defensive.
                                ...
                                { // The marker is not displayed when AI is defensive.
                                    thisImage.enabled = false;
                                }
                                break;
                            case 1: // Offensive.
        ...
        void Set_Position()
        {
            ...
            { // Behind of the camera.
                currentPos.z = -100.0f;
                if (currentPos.x > Screen.width * 0.5f)
                { // Left side.
                    currentPos.x = Side_Offset * resolutionOffset;
                    thisTransform.localRotation = leftRot;
                }
        ...
        { // Called from "ID_Settings_CS" in the top object of the tank.
            if (idScript == null)
            { // This marker is not used yet.
                idScript = tempIDScript;
                Get_Tank_Components();
            }
            else
            { // This marker is already used by other tank.
