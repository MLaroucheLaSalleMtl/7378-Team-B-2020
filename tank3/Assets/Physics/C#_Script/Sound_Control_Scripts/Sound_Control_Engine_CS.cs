	...
	public class Sound_Control_Engine_CS : MonoBehaviour
	{
		...
		*/
		...
		void Initial_Settings()
		{
			...
			if (Left_Reference_Rigidbody == null) { // The left reference wheel has been lost by modifying.
			...
			if (Right_Reference_Rigidbody == null) { // The right reference wheel has been lost by modifying.
        ...
        void Engine_Sound()
		{
            ...
            // Set the rate.
            if (driveScript.Input_Type != 10 && driveScript.Stop_Flag == false && (driveScript.L_Input_Rate == driveScript.R_Input_Rate))
            { // Not AI tank, and the tank should be doing pivot-turn.
                targetRate = (Left_Velocity + Right_Velocity) / 2.0f / (driveScript.Max_Speed * driveScript.Pivot_Turn_Rate);
            }
            else
            {
